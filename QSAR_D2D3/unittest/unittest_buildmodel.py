...
class TestBuildModel(unittest.TestCase):
    ...
    def startUp1(self):
        ...
        train_topo_descs, topo_index, topo_names = prune_topo_descs(self.mode, train_topo_descs,
                                                                    train_acts, self.output_dir, output_ext)
        train_phore_descs = calc_phore_descs(train_mols)
        train_phore_descs, phore_sigbits, phore_names = prune_phore_descs(train_phore_descs, self.output_dir, output_ext)
    ...
    def startUp2(self):
        ...
        train_mols, train_names, train_acts, output_ext = self.startUp()
        input_data = read_mols(self.mode, self.method, "pred", output_ext, datadir=unittest_data_dir+"/data4buildmodels",
                               modeldir=reference)
    ...
    def test_calc_appdom(self):
        ...
        """
        train_mols, train_names, train_acts, output_ext = self.startUp()
        self.assertEqual(len(calc_appdom(train_mols, self.output_dir, output_ext)), 2)

        ad_fps, ad_rad = calc_appdom(train_mols, self.output_dir, output_ext)
    ...
    def test_prune_topo_descs(self):
        ...
        train_mols, train_names, train_acts, output_ext = self.startUp()
        train_topo_descs = calc_topo_descs(train_mols)
        self.assertEqual(len(prune_topo_descs(self.mode, train_topo_descs, train_acts, self.output_dir, output_ext)), 3)
        a, b, c = prune_topo_descs(self.mode, train_topo_descs, train_acts, self.output_dir, output_ext)
    ...
    def test_calc_chir_descs(self):
        """ Test calc_chir_descs function """
        # Create mock molecular data
        mols = [
            Chem.MolFromSmiles('C'),  # Methane: No chiral center
            Chem.MolFromSmiles('CCC(=O)C(CC(C)N(C)C)(c1ccccc1)c1ccccc1'),  # Methadone
            Chem.MolFromSmiles('CCC(=O)C(C[C@@H](C)N(C)C)(c1ccccc1)c1ccccc1'),  # R-Methadone
            Chem.MolFromSmiles('CCC(=O)C(C[C@H](C)N(C)C)(c1ccccc1)c1ccccc1'),  # R-Methadone
            # Sample compound with 3 chiral centers
            Chem.MolFromSmiles('Cc3cc(C2OOC1(C)C=CC(=O)CC1(C)O2)on3'),
            Chem.MolFromSmiles('Cc3cc([C@H]2OO[C@]1(C)C=CC(=O)C[C@]1(C)O2)on3'),
            Chem.MolFromSmiles('Cc3cc([C@@H]2OO[C@@]1(C)C=CC(=O)C[C@@]1(C)O2)on3'),
            Chem.MolFromSmiles('Cc3cc([C@H]2OO[C@@]1(C)C=CC(=O)C[C@]1(C)O2)on3'),
            Chem.MolFromSmiles('Cc3cc([C@H]2OO[C@@]1(C)C=CC(=O)C[C@@]1(C)O2)on3'),
            Chem.MolFromSmiles('Cc3cc([C@@H]2OO[C@]1(C)C=CC(=O)C[C@@]1(C)O2)on3'),
        ...
        expected_descs = np.array([
            [0, 0], [1, 0], [1, 1.0], [1, -1.0], [3, 0], [3, -0.07142857142857142],
            [3, 0.07142857142857142], [3, -0.6428571428571429], [3, 0.42857142857142855],
            [3, 0.6428571428571429], [3, -0.42857142857142855], [3, 1.0], [3, -1.0]
        ])

        descs = calc_chir_descs(mols)

        # Assert that the descriptors are as expected
        np.testing.assert_array_almost_equal(descs, expected_descs, decimal=5)
    ...
    def test_prune_phore_descs(self):
        ...
        train_mols, train_names, train_acts, output_ext = self.startUp()
        train_phore_descs = calc_phore_descs(train_mols)
        self.assertEqual(len(prune_phore_descs(train_phore_descs, self.output_dir, output_ext)), 3)
        train_phore_descs, phore_sigbits, phore_names = prune_phore_descs(train_phore_descs, self.output_dir, output_ext)
    ...
    def test_build_model(self):
        ...
        model, model_score, best_params = build_model(self.mode, self.method, self.rand_states[0],
                                                      train_descs, train_acts, self.output_dir, output_ext)
        ...
        # # BRBV updated reference for the new parameters
        # g = open(reference + '/model_ref2.dat', 'rb')
        # self.assertEqual(str(pickle.load(f)), str(pickle.load(g)))
        # f.close()
        # g.close()

        with open(self.output_dir + ('/model_%s.dat' % output_ext), 'rb') as f, \
                open(reference + '/model_ref2.dat', 'rb') as g:
            self.assertEqual(str(pickle.load(f)), str(pickle.load(g)))
    ...
    def test_predict_model(self):
        ...
        # BRBV updated reference for the new parameters
        a = np.load(reference + "/predictmodel2.npy", allow_pickle=True)
        ref = dict(enumerate(a.flatten()))[0]
        ...
        test_r2, test_rmse, test_mse = predict_model(model,train_names, train_acts ,test_descs, test_acts, test_names, self.rand_split[0],
